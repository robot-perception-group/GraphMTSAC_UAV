...
class BlimpFeature(FeatureAbstract):
    ...
    def extract(self, s, a=0):
        ...
        # Nav planar:
        x = self.compute_featureProx(
            error_posNav[:, 0:2],
            self.proxScaleNav,
            self.TriggerThresh,
            scale=self.scaleProx,
        )
        features.append(x)
        ...
        x = -self.compute_featurePosNorm(
            self.reset_dist - dist_to_center, scale=self.scalePos
        )
        features.append(x)
        ...
        x = (
            self.compute_featureAngNorm(error_navNextHeading, scale=self.scaleAng)
            / dist
        )
        features.append(x)
...
@torch.jit.script
def linear_feature(distance, scale=1.0):
    # type: (Tensor, float) -> Tensor
    return -scale * distance


@torch.jit.script
def quad_feature(distance, scale=1.0):
    # type: (Tensor, float) -> Tensor
    return 1.0 / (1.0 + (scale * distance) ** 2)


@torch.jit.script
def binary_feature(distance, scale):
    # type: (Tensor, float) -> Tensor
    return torch.where(
        distance < scale, torch.ones_like(distance), torch.zeros_like(distance)
    )


class QuadcopterFeature(FeatureAbstract):
    def __init__(self, env_cfg, device) -> None:
        super().__init__()

        self.verbose = env_cfg["feature"]["verbose"]
        self.names = env_cfg["feature"]["names"]

        self.device = device
        self.dim = len(self.names)

        pos_lim = float(env_cfg["task"]["pos_lim"])
        vel_lim = float(env_cfg["task"]["vel_lim"])
        ang_lim = float(torch.pi)
        angvel_lim = float(env_cfg["task"]["avel_lim"])
        self.lims = [pos_lim, vel_lim, ang_lim, angvel_lim]

        with_thrust = int(env_cfg["task"]["with_thrust"])
        with_yaw = int(env_cfg["task"]["with_yaw"])
        self.withs = with_thrust + with_yaw

    def extract(self, s, a):
        return self._extract(s, a, self.lims)

    @torch.jit.script
    def _extract(s, act, lims):
        ...
        # raw obs
        # robot_ang = s[:, 0:3]  
        robot_angvel_rp = s[:, 3:5]  
        robot_angvel_y = s[:, 5]  
        
        prev_act = s[:, 6:9]  

        # error_ang = check_angle(robot_ang - goal_ang)
        error_ang_rp = s[:, 9:11]
        error_ang_y = s[:, 11] 

        # angle rp and angle rp cd
        d = torch.norm(error_ang_rp, p=2, dim=-1)
        x = 0.5 * quad_feature(d, 3.3) + 0.5 * binary_feature(d, 0.1)
        features.append(x)

        x = 0.5 * quad_feature(d, 10.0) + 0.5 * binary_feature(d, 0.01)
        features.append(x)

        # angle y and angle y cd
        d = torch.abs(error_ang_y) / 3.33
        x = 0.5 * quad_feature(d, 3.3) + 0.5 * binary_feature(d, 0.1)
        features.append(x)
        ...
        # no spinning rp and y
        d = torch.norm(robot_angvel_rp, p=2, dim=-1)
        x = 0.5 * quad_feature(d, 3.3) + 0.5 * binary_feature(d, 0.1)
        features.append(x)

        d = 10*torch.abs(robot_angvel_y)
        x = 0.5 * quad_feature(d, 3.3) + 0.5 * binary_feature(d, 0.1)
        features.append(x)

        # action smoothness
        d = torch.norm(prev_act - act, p=2, dim=-1)
        x = linear_feature(d, 1.0)
        features.append(x)

        return torch.stack(features, -1)


class TailsitterFeature(FeatureAbstract):
    def __init__(self, env_cfg, device) -> None:
        super().__init__()
        ...
        pos_lim = env_cfg["task"]["pos_lim"]
        vel_lim = env_cfg["task"]["vel_lim"]
        ang_lim = torch.pi
        angvel_lim = env_cfg["task"]["avel_lim"]
        self.lims = [pos_lim, vel_lim, ang_lim, angvel_lim]
    ...
    def _extract(s, a, lims):
        ...
        # raw obs
        robot_quat = s[:, 0:4]
        robot_vel = s[:, 4:7]  # local, scaled
        robot_angvel = s[:, 7:10]  # local, scaled

        # error_ang = s[:, 10:13]   # scaled
        # error_vf = s[:, 13]  # local, scaled

        # velocity forward
        # e = torch.abs(error_vf)
        # x = 0.5*quad_feature(e, 10.0) + 0.5*binary_feature(e, 0.05)
        # features.append(x)

        # angle roll
        # e = torch.abs(error_ang)
        # x = 0.5*quad_feature(e[...,0], 10.0) + 0.5*binary_feature(e[...,0], 0.05)
        # features.append(x)

        # angle pitch
        # x = 0.5*quad_feature(e[...,1], 10.0) + 0.5*binary_feature(e[...,1], 0.05)
        # features.append(x)

        # angle yaw
        # x = 0.5*quad_feature(e[...,2], 10.0) + 0.5*binary_feature(e[...,2], 0.05)
        # features.append(x)

        # uprightness
        ups = quat_axis(robot_quat, 2)
        e = 1 - ups[..., 2]
        x = 0.5 * quad_feature(torch.abs(e), 10.0) + 0.5 * binary_feature(e, 0.05)
        features.append(x)

        # levelness
        # front = quat_axis(robot_quat, 0)
        # e = 1 + front[..., 2]
        # x = 0.5*quad_feature(torch.abs(e), 10.0) + 0.5*binary_feature(e, 0.05)
        # features.append(x)

        # no spinning bonus
        d = torch.norm(robot_angvel, p=2, dim=-1)
        x = 0.5 * quad_feature(d, 10.0) + 0.5 * binary_feature(d, 0.05)
        features.append(x)

        # no moving bonus
        d = torch.norm(robot_vel, p=2, dim=-1)
        x = 0.5 * quad_feature(d, 10.0) + 0.5 * binary_feature(d, 0.05)
        features.append(x)
...
def feature_constructor(env_cfg, device):
    if "pointer" in env_cfg["env_name"].lower():
        ...
        return TailsitterFeature(env_cfg, device)
    elif "quadcopter" in env_cfg["env_name"].lower():
        return QuadcopterFeature(env_cfg, device)
