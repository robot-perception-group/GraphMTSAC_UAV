...
class FIFOBuffer:
    ...
    def add(self, sa, dones):
        # sa: state and action pair at current time step [N, S+A]
        # dones: clear buffer of the respective env if done is true [N]
        self.buf = torch.cat([self.buf[1:], sa[None, ...]])

        # correct by dones
        new_done = (~dones[:, None].bool()).repeat(self.stack_size, 1, self.traj_dim)
        self.buf = new_done * self.buf

    def get(self):
        buf = self.buf.permute(1, 2, 0) #[obs+act, stack]
