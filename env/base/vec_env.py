...
class VecEnv:
    def __init__(self, cfg):
        ...
        self.log_rewards = cfg["task"].get("log_rewards_dict", {"default": ["return"]})[
            self.single_task
        ]
...
class MultiTaskVecEnv(VecEnv):
    def __init__(self, cfg):
        super().__init__(cfg)
        self.task = task_constructor(cfg, self.sim_device)

        self.max_difficulty = cfg["task"].get("difficulty", 0)
        self.difficulty = self.max_difficulty

        self._task_mode = cfg["task"].get("task_mode", "train")

    def set_difficulty(self, difficulty=None):
        if difficulty is not None:
            self.difficulty = difficulty

        self.difficulty = np.clip(self.difficulty, 0, self.max_difficulty)

    def set_task(self, task="train"):
        if task == "eval":
            self.set_difficulty(self.max_difficulty)
