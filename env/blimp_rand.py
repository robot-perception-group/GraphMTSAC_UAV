...
from common.misc.pid import (
    BlimpPositionControl,
    BlimpHoverControl,
    BlimpVelocityControl,
    BlimpBackwardControl,
)

from .base.vec_env import VecEnv
from .base.goal import RandomNavWayPoints, FixWayPoints
...
class BlimpRand(VecEnv):
    ...
    def get_obs(self, env_ids=None):
        ...
        # robot angle
        euler = euler_from_quat(self.rb_rot[env_ids, body_id, :])
        euler = check_angle(euler)

        self.id_ang[0] = d
        self.obs_buf[env_ids, d] = euler[..., 0]
        d += 1
        self.id_ang[1] = d
        self.obs_buf[env_ids, d] = euler[..., 1]
        d += 1
        self.obs_buf[env_ids, d] = euler[..., 2]
        ...
        rel_pos = (
            self.rb_pos[env_ids, 0] - self.wp.get_pos_nav(self.wp.idx + 1)[env_ids]
        )
        d += 1  # 27
        self.obs_buf[env_ids, d] = rel_pos[:, 0]
        d += 1
    ...
    def reset(self):
        ...
        self.rb_rot[env_ids, 0, :] = quat_from_euler(rotations)
    ...
    def _add_thrust_lines(self, num_lines, line_colors, line_vertices, envs):
        ...
        s = 1
        euler = euler_from_quat(self.rb_rot[:, 3, :])
        act = self.actions_tensor[:, 3, 0:3]
        f1, f2, f3 = localToGlobalRot(euler, act)

        euler = euler_from_quat(self.rb_rot[:, 7, :])
        act = self.actions_tensor[:, 7, 0:3]
        g1, g2, g3 = localToGlobalRot(euler, act)
    ...
    def _add_drag_lines(self, num_lines, line_colors, line_vertices, envs):
        ...
        s = 50
        idx = 12
        euler = euler_from_quat(self.rb_rot[:, idx, :])
        act = self.actions_tensor[:, idx, 0:3]
        f1, f2, f3 = localToGlobalRot(euler, act)
...
def simulate_aerodynamics(
    ...
    r, p, y = euler_from_quat_multi(rb_rot[:, drag_bodies, :])
    a, b, c = globalToLocalRot2(
        r,
        p,
        y,
...
def compute_point_reward(
    ...
    # reset
    reset = torch.where(sqr_dist > reset_dist**2, torch.ones_like(reset_buf), reset_buf)
    # reset = torch.where(
